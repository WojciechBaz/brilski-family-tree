...
const ORBIT_RINGS = [
  { radius: 82, duration: 26, items: ["1844", "1870", "1901"] },
  { radius: 128, duration: 42, items: ["1850", "1880", "1910", "1930"] },
  { radius: 178, duration: 62, items: ["1950", "1975", "1990", "2020"] },
];
...
export default function FamilyTreeOrbit({ onEnterTree }) {
  return (
    <div className="relative overflow-hidden rounded-[2.2rem] border border-[#b68a57]/24 bg-[linear-gradient(180deg,rgba(70,46,29,0.58),rgba(25,15,10,0.72))] p-6 shadow-[inset_0_0_40px_rgba(0,0,0,0.25)] backdrop-blur-sm">
      <div className="text-[11px] uppercase tracking-[0.32em] text-[#d9bf8e]/62">
        Explore the lineage
      </div>

      <div className="relative mt-4 h-[360px] overflow-hidden rounded-[1.8rem] border border-[#b68a57]/14 bg-[radial-gradient(circle_at_center,rgba(210,170,110,0.08),transparent_28%),linear-gradient(180deg,rgba(44,28,18,0.62),rgba(22,14,9,0.7))]">
        ...
        <motion.div
          className="absolute left-1/2 top-1/2 h-[118px] w-[118px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#d7bb86]/22 bg-[radial-gradient(circle_at_center,rgba(234,215,176,0.18),rgba(58,38,24,0.8))] shadow-[0_0_24px_rgba(234,215,176,0.06)]"
          animate={{
            scale: [1, 1.035, 1],
            boxShadow: [
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          className="absolute left-1/2 top-1/2 z-10 flex h-[96px] w-[96px] -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-[#e4c58f]/28 bg-[#2f1e13]/82 px-4 text-center text-lg font-medium text-[#f2dfb7] shadow-[0_0_18px_rgba(234,215,176,0.08)] transition hover:bg-[#3a2618]"
        >
          <span className="font-serif leading-tight">Family Tree</span>
        </motion.button>

        <motion.div
          className="absolute left-1/2 top-[19%] h-3.5 w-3.5 -translate-x-1/2 rounded-full bg-[#ead7b0]/85 shadow-[0_0_16px_rgba(234,215,176,0.4)]"
          animate={{ opacity: [0.5, 1, 0.5], scale: [1, 1.25, 1] }}
          transition={{ duration: 2.8, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute left-[24%] top-1/2 h-2.5 w-2.5 -translate-y-1/2 rounded-full bg-[#ead7b0]/48"
          animate={{ opacity: [0.25, 0.75, 0.25] }}
          transition={{ duration: 3.4, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute right-[24%] top-1/2 h-2.5 w-2.5 -translate-y-1/2 rounded-full bg-[#ead7b0]/48"
          animate={{ opacity: [0.25, 0.75, 0.25] }}
          transition={{
            duration: 3.4,
            repeat: Infinity,
